...
codis_ebre = c("43052", "43133", "43903", "43904", "43901", "43102", "43013", "43149", "43008", "43006", "43025", "43104", "43906", "43155", 
               "43136", "43014", "43004", "43044", "43902", "43156", "43138", "43068", "43078", "43063", "43062", "43077", "43094", "43093", 
               "43019", "43060", "43152", "43065", "43150", "43026", "43067", "43084", "43125", "43099", "43121", "43177", "43064", "43032", 
               "43106", "43022", "43071", "43048", "43018", "43056", "43110", "43175", "43041") %>% 
...
print(
  (ggplot(data = df, aes(x=as.Date(date)+1)) +
     ...
     ggtitle(label = "Terres de l'Ebre", subtitle = "Tendència desestacionalitzada de casos nous diaris") + 
    ...
    ggsave(filename = str_c("ebre/trend ext.png"), device = "png", dpi = 200, width = 7, height = 4)
)
...
print(
  (ggplot(data = df, aes(x=as.Date(date)+1)) +
     ...
     ggtitle(label = "Terres de l'Ebre", subtitle = "Incidència acumulada darrers 14 dies") + 
     scale_y_continuous(name = "IA14") + expand_limits(y = 0) +
     theme(axis.title.y.right = element_text(color = "#69b3a2"), axis.title.x=element_blank()) + 
     geom_label(data=as.data.frame(endvalue),aes(x=maxdate+(datelength/50),y=endvalue), label = endvalue, label.size = NA, hjust = 0, fill=NA) + 
     scale_x_date(date_breaks = 'week', date_labels = '%d\n%b', name = element_blank())) %>% 
    ggsave(filename = str_c("ebre/ia14.png"), device = "png", dpi = 200, width = 7, height = 4)
)
...
print(
  (ggplot(data = df, aes(x=as.Date(date)+1)) +
     ...
     ggtitle(label = "Terres de l'Ebre", subtitle = "Índex de risc de rebrot") + 
     scale_y_continuous(name = "EPG") + expand_limits(y = 0) +
     theme(axis.title.y.right = element_text(color = "#69b3a2"), axis.title.x=element_blank()) + 
     geom_label(data=as.data.frame(endvalue),aes(x=maxdate+(datelength/50),y=endvalue), label = endvalue, label.size = NA, hjust = 0, fill=NA) + 
     scale_x_date(date_breaks = 'week', date_labels = '%d\n%b', name = element_blank())) %>% 
    ggsave(filename = str_c("ebre/epg.png"), device = "png", dpi = 200, width = 7, height = 4)
)
...
print(
  (ggplot(data = df, aes(x=as.Date(date)+1)) +
     ...
     geom_label(data=as.data.frame(endvalue),aes(x=maxdate+(datelength/50),y=endvalue), label = endvalue, label.size = NA, hjust = 0, fill=NA) + 
     scale_x_date(date_breaks = 'week', date_labels = '%d\n%b', name = element_blank())) %>% 
    ggsave(filename = str_c("ebre/trend.png"), device = "png", dpi = 200, width = 7, height = 4)
)
